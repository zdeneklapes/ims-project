...
struct SystemRLC {
    ...
    double ResFrequency() { return 1 / (2 * PI * std::sqrt(L.Value() * C.Value())); }
};
...
void Sample1() {  // output from model (form suitable for GNUplot)
    Print("%g %g %g %g %g %g %g\n", T.Value(), Uin.Value(), rlc1.Uout.Value(), rlc1.Current2.Value(), Freq.Value(),
          rlc1.LastMax.Value(), rlc1.TotalMax.Value());
}
