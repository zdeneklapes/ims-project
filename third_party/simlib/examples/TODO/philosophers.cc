...
struct DiningPhilosophers {
    ...
    class Philosopher : public Process {
       ...
       public:
        Philosopher(int id, Fork &l, Fork &r) : state(THINKING), number(id), left(l), right(r) {}
    };
    ...
    DiningPhilosophers() {
        for (int i = 0; i < N; i++) p[i] = new Philosopher(i + 1, f[i], f[(i + 1) % N]);
    }

    void ActivateAll() {
        for (int i = 0; i < N; i++) p[i]->Activate();  // start behavior
    }
