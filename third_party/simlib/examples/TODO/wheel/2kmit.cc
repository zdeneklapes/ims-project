...
struct Mass : public aContiBlock1 {
    ...
    Mass(Input force, double m, double pos0)
        : aContiBlock1(force), mass(m), speed(-g + force / mass), position(speed, pos0) {}
    double Value() { return position.Value(); }
};

struct Spring : aContiBlock2 {
    double K, L0;
    Spring(Input p1, Input p2, double k, double l0) : aContiBlock2(p1, p2), K(k), L0(l0) {}
    double Value() {
...
struct System2 {
    ...
    //
    System2()
        : m1(f1 - f2, M1, y1), m2(f2, M2, y2), f1(y0, m1.position, K1, L1), f2(m1.position, m2.position, K2, L2) {}
};
...
void Sample() {
    Print("%g  %g  %g  %g  %g\n", T.Value(), s.m1.position.Value(), s.m2.position.Value(), s.m1.speed.Value(),
          s.m2.speed.Value());
}
