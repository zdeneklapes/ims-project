...
class Pendulum {
    ...
    Integrator x;   // angle
   public:
    Pendulum(double len) : L(len), x1(-g / L * Sin(x)), x(x1) {}
    ...
    void PrintValues(ostream& out) { out << Time << ' ' << x.Value() << ' ' << x1.Value() << "\n"; }
};

Pendulum k(Length);

// output model state
void Sample() { k.PrintValues(cout); }
...
int main() {
    ...
    while (1) {
        ...
        cerr << "# angle_0 = " << angle << " rad,  \t av_0 = " << setprecision(16) << av << " rad.s^{-1}\n";
