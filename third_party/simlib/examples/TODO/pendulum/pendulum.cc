...
struct param {
    ...
    {1, 0, 10}, {2, 0, 12}, {0, 1.98090888230, 32}, {0, 1.98090888231, 35}, {0, 2.2, 8},
};
...
class Pendulum {
    ...
    Integrator a;   // angle
   public:
    Pendulum(double length) : L(length), av(-g / L * Sin(a)), a(av) {}
    ...
    void PrintValues(std::ostream &out) { out << Time << ' ' << a.Value() << ' ' << av.Value() << "\n"; }
};
