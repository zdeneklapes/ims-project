 ...
 */
FuzzySet::FuzzySet(const char *name, double min, double max) : n(0), xmin(min), xmax(max) { Name = strdup(name); }
 ...
 * @param max Maximal value of universum.<br>Horní mez univerza.
 */
FuzzySet::FuzzySet(const char *name, double min, double max, const FuzzyMembershipFunction &m1)
...
FuzzySet::FuzzySet(const char *name, double min, double max, const FuzzyMembershipFunction &m1,
                   const FuzzyMembershipFunction &m2, const FuzzyMembershipFunction &m3,
                   const FuzzyMembershipFunction &m4, const FuzzyMembershipFunction &m5)
    : n(0), xmin(min), xmax(max) {
    Name = strdup(name);
    add(m1);
    add(m2);
...
const FuzzyMembershipFunction *FuzzySet::operator[](int i) const {
    if (unsigned(i) >= n) SIMLIB_error("FuzzySet[]: index out of range");
    return array[i];
}
...
double FuzzySet::Membership(int i, double x) const {
    if (unsigned(i) >= n) SIMLIB_error("FuzzySet::Membership: index out of range");
...
FuzzySet::~FuzzySet() {
    ...
    for (unsigned i = 0; i < n; i++) delete array[i];  // clean memory
    delete[] Name;
}
...
FuzzyVariable::FuzzyVariable(const FuzzySet &t) : n(t.count()) {
    ...
    if (activeFuzzyBlock != 0) registerOwner(activeFuzzyBlock);
...
const FuzzyMembershipFunction *FuzzyVariable::mf(int i) const {
    if (unsigned(i) >= n) SIMLIB_error("FuzzyVariable::mf(i) index out of range");
    return (*m)[i];
}

/** It gets center of i-th member function.<br>Vrací støed i-té funkce pøíslu¹nosti. */
double FuzzyVariable::center(int i) const { return mf(i)->center(); }

/** It gets i-th word value.<br>Vrací i-tou slovní hondotu. */
const char *FuzzyVariable::wordValue(int i) const { return mf(i)->wordValue(); }

/** Get/set fuzzy value.<br>Vra»/nastav fuzzy hodnotu. */
double &FuzzyVariable::operator[](int i) const {
    if (unsigned(i) >= n) SIMLIB_error("FuzzyVariable::[i] index out of range");
    return mval[i];
}
...
double FuzzyVariable::SetMembership(int i, double x) { return mval[i] = m->Membership(i, x); }
