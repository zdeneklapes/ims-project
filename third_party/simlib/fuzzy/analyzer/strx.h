...
class StrX {
   public:
     ...
     */
    StrX(const XMLCh* const value) : ascii(NULL), unicode(NULL) { setValue(value); }
    /**
     * It creates a copy according to ascii value.<br>
     * Vytvoøí kopii podle ascii hodnoty.
     */
    StrX(const char* const value) : ascii(NULL), unicode(NULL) { setValue(value); }
    ...
    const char* toAscii() const { return ascii; }
    /** It returns unicode strign.<br>Vrátí unicode øetìzec. */
    const XMLCh* toUnicode() const { return unicode; }
    /** Comparising with ascii string.<br>Porovnání s ascii øetìzcem. */
    bool equals(const char* str) { return XMLString::compareString(ascii, str) == 0; }
    /** Comparising with unicode string.<br>Porovnání s unicode øetìzcem. */
    bool equals(const XMLCh* str) { return XMLString::compareString(unicode, str) == 0; }
