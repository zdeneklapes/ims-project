 ...
 */
FuzzyMembershipFunction::FuzzyMembershipFunction(const char *name) : Name(NULL), defValues(0) { Name = strdup(name); }
 ...
 */
FuzzySingleton::FuzzySingleton(const char *name, double a) : FuzzyMembershipFunction(name), x0(a) {
    defValues = 1;
 ...
 * @param name Word value for this function.<br>Slovní hodnota pro tuto funkci
 */
FuzzySingleton::FuzzySingleton(const char *name) : FuzzyMembershipFunction(name) {
    TRACE(printf("FuzzySingleton::FuzzySingleton(%s)\n", name));
}
...
FuzzySingleton *FuzzySingleton::clone() const { return new FuzzySingleton(*this); }
 ...
 * @param name Word value for this function.<br>Slovní hodnota pro tuto funkci
 */
FuzzyTriangle::FuzzyTriangle(const char *name) : FuzzyMembershipFunction(name) {
    TRACE(printf("FuzzyTriangle::FuzzyTriangle(%s)\n", name));
}
...
/** It duplicates object.<br>Duplikuje objekt. */
FuzzyTriangle *FuzzyTriangle::clone() const { return new FuzzyTriangle(*this); }

/** This adds next definition value.<br>Pøidá dal¹í definièní hodnotu. */
void FuzzyTriangle::addDefValue(double value) {
    if (isComplete()) SIMLIB_error("Bad triangle membership function definition. Too much definition values.");
    switch (defValues++) {
        case 0:
            x0 = value;
            break;
...
FuzzyTrapez::FuzzyTrapez(const char *name, double a, double b, double c, double d)
    ...
    if (a > b || b > c || c > d) SIMLIB_error("Bad trapez membership function definition.");
}
 ...
 * @param name Word value for this function.<br>Slovní hodnota pro tuto funkci
 */
FuzzyTrapez::FuzzyTrapez(const char *name) : FuzzyMembershipFunction(name) {
    TRACE(printf("FuzzyTrapez::FuzzyTrapez(%s)\n", name));
}
...
/** It duplicates object.<br>Duplikuje objekt. */
FuzzyTrapez *FuzzyTrapez::clone() const { return new FuzzyTrapez(*this); }

/** This adds next definition value.<br>Pøidá dal¹í definièní hodnotu. */
void FuzzyTrapez::addDefValue(double value) {
    if (isComplete()) SIMLIB_error("Bad trapez membership function definition. Too much definition values.");
    switch (defValues++) {
        case 0:
            x0 = value;
            break;
 ...
 * @param name Word value for this function.<br>Slovní hodnota pro tuto funkci
 */
FuzzyGauss::FuzzyGauss(const char *name) : FuzzyMembershipFunction(name) {
    TRACE(printf("FuzzyGauss::FuzzyGauss(%s)\n", name));
}
...
double FuzzyGauss::Membership(double x) const {
    ...
    if (twoSqrSigma == 0)
        ...
        return exp(-((x - c) * (x - c)) / twoSqrSigma);
}

/** It duplicates object.<br>Duplikuje objekt. */
FuzzyGauss *FuzzyGauss::clone() const { return new FuzzyGauss(*this); }

/** This adds next definition value.<br>Pøidá dal¹í definièní hodnotu. */
void FuzzyGauss::addDefValue(double value) {
    if (isComplete()) SIMLIB_error("Bad Gaussian membership function definition. Too much definition values.");
    switch (defValues++) {
        case 0:
            c = value;
            break;
        case 1:
            sigma = value / 3;
            twoSqrSigma =
                2 * sigma * sigma;  // this variable must! be reinitialized everywhere, when sigma
                                    //      if (sigma == 0)
 ...
 */
FuzzyGauss2::FuzzyGauss2(const char *name, double leftCenter, double leftRadius, double rightCenter, double rightRadius)
 ...
 * @param name Word value for this function.<br>Slovní hodnota pro tuto funkci
 */
FuzzyGauss2::FuzzyGauss2(const char *name) : FuzzyMembershipFunction(name) {
    TRACE(printf("FuzzyGauss2::FuzzyGauss2(%s)\n", name));
}

/** It duplicates object.<br>Duplikuje objekt. */
FuzzyGauss2 *FuzzyGauss2::clone() const { return new FuzzyGauss2(*this); }
...
double FuzzyGauss2::Membership(double x) const {
    ...
    double y2;
    if (twoSqrSigmaL == 0)
        y1 = (x < leftCenter) ? 0.0 : 1.0;
    else
        y1 = (x < leftCenter) ? exp(-(x - leftCenter) * (x - leftCenter) / twoSqrSigmaL) : 1.0;

    if (twoSqrSigmaR == 0)
        y2 = (x > rightCenter) ? 0.0 : 1.0;
    else
        y2 = (x > rightCenter) ? exp(-(x - rightCenter) * (x - rightCenter) / twoSqrSigmaR) : 1.0;
...
double FuzzyGauss2::center() const {
    if (leftCenter <= rightCenter)
        return (leftCenter - leftSigma * 3 + 2 * leftCenter + 2 * rightCenter + rightCenter + rightSigma * 3) / 6;
    else
...
void FuzzyGauss2::addDefValue(double value) {
    ...
    switch (defValues++) {
        ...
        case 3:
            rightSigma = value / 3;
            twoSqrSigmaR =
                2 * rightSigma * rightSigma;  // this variable must! be reinitialized everywhere, when sigma is
            break;
        default:
            break;
    }
