 ...
 */
FuzzyRuleFactory *FuzzyInferenceRules::createRuleFactory() { return new FuzzyRuleFactory(this); }
 ...
 */
FuzzyIIORules::FuzzyIIORules() { init(); }
...
void FuzzyIIORules::add(FuzzyRule *rule, bool release /*=true*/) {
    if ((typeid(*(rule->left->L)) != typeid(FPair)) || (typeid(*(rule->left->R)) != typeid(FPair))) {
    ...
    if ((typeid(*(L->var)) != typeid(FuzzyInput)) || (typeid(*(R->var)) != typeid(FuzzyInput)) || !L->eq || !R->eq) {
        SIMLIB_error("FuzzyIIORules::add: Tree in parameter (FuzzyRule) has bad structure!");
    }
...
bool FuzzyIIORules::isAllCreated() { return (operation != NULL) && (outWV != NULL); }
...
void FuzzyIIORules::addFuzzyInput(FuzzyInput *in) {
    if (inputs < MAX_INPUTS) {
        inputs++;
        FuzzyInferenceRules::addFuzzyInput(in);
    } else {
        SIMLIB_error("FuzzyIIORules: can not store more than 2 fuzzy inputs.");
    }
...
void FuzzyIIORules::addFuzzyOutput(FuzzyOutput *out) {
    if (outputs < MAX_OUTPUTS) {
        outputs++;
        FuzzyInferenceRules::addFuzzyOutput(out);
    } else {
        SIMLIB_error("FuzzyIIORules: can not store more than 1 fuzzy output.");
    }
    if (isComplete() && !isAllCreated()) createVectors();
}
 ...
 */
bool FuzzyIIORules::isComplete() { return (inputs == MAX_INPUTS) && (outputs == MAX_OUTPUTS); }
 ...
 */
void FuzzyIIORules::add(const FuzzyInferenceRules::Operations operation, int in1WVIndex, int in2WVIndex,
                        int outWVIndex) {
 ...
 */
void FuzzyIIORules::add(const FuzzyInferenceRules::Operations operation, const char *in1WordValue,
                        const char *in2WordValue, const char *outWordValue) {
 ...
 * @see addFuzzyOutput(FuzzyOutput *out)
 */
bool FuzzyGeneralRules::isComplete() { return (in.size() > 0) && (out.size() > 0); }
 ...
 */
void FuzzyGeneralRules::add(FuzzyRule *rule, bool release /*=true*/) { rules.push_back(rule); }
