...
void Rules::addFuzzyInput(FuzzyInput *input) {
    if (Rules::rules == NULL) {
        Rules::rules = new FuzzyGeneralRules();
    }
 ...
 */
void Rules::addNewRule() { Rules::rules->add(Rules::factory->createRule()); }
 ...
 */
void FuzzyBlock::EndConstructor() { activeFuzzyBlock = where; }
 ...
 */
void FuzzyBlock::Register(FuzzyVariable *fs) { vlist.push_back(fs); }
...
void FuzzyBlock::Evaluate() {
    ...
    for (i = vlist.begin(); i != vlist.end(); i++) (*i)->Init();  // fuzzify input, init output
    Behavior();                                                   // execute rules, aggregate output
    // for_each
    for (i = vlist.begin(); i != vlist.end(); i++) (*i)->Done();  // defuzzify outputs
}
