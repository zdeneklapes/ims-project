 ...
 */
FuzzyRule::~FuzzyRule() {
    if (left != NULL) for_each(right.begin(), right.end(), del);
 ...
 */
void FuzzyRule::addLeft(FOperation *left) { this->left = left; }

/**
 * It adds one command into list. <br> Pøidá dal¹í pøíkaz do seznamu.
 */
void FuzzyRule::addRight(FPair *right) { this->right.push_back(right); }
 ...
 */
double FPair::getValue() { return (*var)[indexWV]; }
