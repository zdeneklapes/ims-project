...
class TestEvent : public Event {
    void Behavior() {
        if (Time < Last_Time) Error("Bad calendar implementation %g < %g", Time, Last_Time);
        Last_Time = Time;
    }
...
double (*distribution[N_D])() = {
    DistributionUni, DistributionExp, DistributionBiased, DistributionBimodal, DistributionTriangular,
};
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        ...
        if (N < 1) _Print("Error: bad argument\n");
    }
