...
euler_step:  // beginning of Euler's step
    ...
    Dprintf(("E_MIN: %g, E_MAX %g", eul_step_coef * SIMLIB_MinStep, eul_step_coef * SIMLIB_StepSize));
    ...
    for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
        ...
        c1 = (denom >= -prec) ? (1.0 + 0.5 * denom) : ((exp(denom) - 1.0) / denom);
        c0 = (ll >= 0) ? (1.0 + denom) : exp(denom);
        // state
