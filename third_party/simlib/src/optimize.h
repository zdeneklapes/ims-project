...
class Param {
   ...
   public:
    Param() : name(0), min(0), max(0), value(min) {}  // for arrays
    Param(const char *n, double a, double b) : name(n), min(a), max(b), value(min) {}
    void Print() const;
...
class ParameterVector {
   ...
   public:
    ...
    void Print() const;
    // Prototypes !!!###
    Param &operator[](const char *name) { return p[search(name)]; }
...
double Optimize_hooke(opt_function_t f, ParameterVector &p, double rho, double epsilon, int itermax);
