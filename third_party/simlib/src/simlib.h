...
class Process : public Entity {
    ...
    enum ProcessStatus_t { _PREPARED = 1, _RUNNING, _INTERRUPTED, _TERMINATED } _status;
...
#define WaitUntil(condition)                     \
    while (_WaitUntil(condition)) /*empty body*/ \
        ;
#endif
...
class aStat : public SimObject {
   ...
   public:
    ...
    double Min() const {             /* test n==0 +exception */
        return min;
    }
    double Max() const { /* test n==0 */
        return max;
    }
...
class Stat : public SimObject {
   ...
   public:
    ...
    double Min() const { /* TODO: test n==0 */
        return min;
    }
    double Max() const { /* test n==0 */
        return max;
...
class TStat : public SimObject {
   ...
   public:
    ...
    unsigned long Number() const { return n; }
    double Min() const { /*TODO: only if(n>0)*/
        return min;
    }
...
class IntegratorContainer {
   ...
   public:
    ...
    static bool isAny(void) { return ListPtr != 0 && !(ListPtr->empty()); }
    // # of elements in the list
    static size_t Size(void) { return (ListPtr != 0) ? (ListPtr->size()) : 0; }
    // return iterator to the first element
    static iterator Begin(void) { return Instance()->begin(); }
    // return iterator to the end (not to the last element!)
    static iterator End(void) { return Instance()->end(); }
...
class SingleStepMethod : public IntegrationMethod {
   ...
   public:
    explicit SingleStepMethod(const char *name) : IntegrationMethod(name), StartMode(false) { /* NOTHING */
    }
...
//! \ingroup simlib
inline void SetMethod(const char *name) { IntegrationMethod::SetMethod(name); }

//! get the name of the method which is used
//! \ingroup simlib
inline const char *GetMethod(void) { return IntegrationMethod::GetMethod(); }

//! set the method which will be used to start given multistep method
//! \ingroup simlib
inline void SetStarter(const char *name, const char *slave_name) { MultiStepMethod::SetStarter(name, slave_name); }
...
//! \ingroup simlib
inline const char *GetStarter(const char *name) { return MultiStepMethod::GetStarter(name); }

//! get the name of the method which is used
//! to start currently used multistep method
//! \ingroup simlib
inline const char *GetStarter(void) { return MultiStepMethod::GetStarter(IntegrationMethod::GetMethod()); }
...
class Iterations : public AlgLoop {
   public:
    Iterations(Input i, double eps, unsigned long max_it, double t_min, double t_max, double t0)
        : AlgLoop(i, eps, max_it, t_min, t_max, t0) { /*nothing*/
    }
    virtual double Value() override;  // returned value
};
...
class Bisect : public AlgLoop {
   public:
    Bisect(Input i, double eps, unsigned long max_it, double t_min, double t_max)
        : AlgLoop(i, eps, max_it, t_min, t_max, t_min) { /*nothing*/
    }
    virtual double Value() override;  // returned value
};
...
class RegulaFalsi : public AlgLoop {
   ...
   public:
    RegulaFalsi(Input i, double eps, unsigned long max_it, double t_min, double t_max)
        : AlgLoop(i, eps, max_it, t_min, t_max, t_min), eps_root(0) { /*nothing*/
    }
    virtual double Value() override;  // returned value
};
...
class Newton : public AlgLoop {
   ...
   public:
    Newton(Input i, double eps, unsigned long max_it, double t_min, double t_max, double t0)
        : AlgLoop(i, eps, max_it, t_min, t_max, t0), eps_root(0), prev_root(0) { /*nothing*/
    }
    virtual double Value() override;  // returned value
};
