...
begin_step:
    ...
    if (ABM_Count <= abm_ord - 2) {
        ...
        Dprintf(("own-method, step = %g, Time = %g", SIMLIB_StepSize, (double)Time));
        ...
        for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
            ...
            (*ip)->SetState(PRED[i] = (*ip)->GetOldState() +
                                      (55.0 * Z[(ind + 3) % abm_ord][i] - 59.0 * Z[(ind + 2) % abm_ord][i] +
                                       37.0 * Z[(ind + 1) % abm_ord][i] - 9.0 * Z[ind][i]) *
        ...
        for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
            (*ip)->SetState((*ip)->GetOldState() + (9.0 * (*ip)->GetDiff() + 19.0 * Z[(ind + 2) % abm_ord][i] -
                                                    5.0 * Z[(ind + 1) % abm_ord][i] + Z[ind][i]) *
                                                       (SIMLIB_StepSize / 24.0));
        }
