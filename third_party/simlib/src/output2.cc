...
void Facility::Output() const {
    ...
    if (tstat.Number() > 0) {
        ...
        if (Time > tstat.StartTime()) Print("|  Average utilization = %-27g       |\n", tstat.MeanValue());
    }
    Print("+----------------------------------------------------------+\n");
...
void Histogram::Output() const {
    ...
    long sum = 0;
    unsigned i;
    for (i = 0; i <= count + 1; i++) sum += dptr[i];
    if (sum == 0) return;
    // Print("+------------+------------+----------+----------+----------+\n");
...
//  Process::Output
//
void Process::Output() const { Print(" PROCESS %-38s %10s \n", Name().c_str(), isCurrent() ? "Current" : " "); }
...
void Queue::Output() const {
    ...
    if (StatN.Number() > 0) {
        ...
        if (StatDT.Number() > 0) {
            ...
            if (StatDT.Number() > 99) Print("|  Standard deviation = %-25g          |\n", StatDT.StdDev());
        }
    }
    Print("+----------------------------------------------------------+\n");
...
void Stat::Output() const {
    ...
    if (n == 0)
        ...
        if (n > 99) Print("|  Standard deviation = %-25g          |\n", StdDev());
    }
    Print("+----------------------------------------------------------+\n");
}
...
void Store::Output() const {
    ...
    Print("| STORE %-50s |\n", Name().c_str());
    Print("+----------------------------------------------------------+\n");
    sprintf(s, " Capacity = %lu  (%lu used, %lu free) ", Capacity(), used, Free());
    Print("| %-56s |\n", s);
    if (tstat.n > 0) {
        sprintf(s, " Time interval = %g - %g ", tstat.StartTime(), (double)Time);
        Print("| %-56s |\n", s);
        ...
        if (Time > tstat.StartTime()) Print("|  Average used capacity = %-30g  |\n", tstat.MeanValue());
    }
    Print("+----------------------------------------------------------+\n");
    if (OwnQueue()) {
...
void TStat::Output() const {
    ...
    if (n == 0)
        ...
        Print("| %-56s |\n", s);
        Print("|  Number of records = %-26ld          |\n", n);
        if (Time > t0) Print("|  Average value = %-25g               |\n", MeanValue());
    }
    Print("+----------------------------------------------------------+\n");
}
