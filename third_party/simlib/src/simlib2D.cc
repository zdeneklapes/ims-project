...
double abs(const Value2D &a) { return sqrt(a._x * a._x + a._y * a._y); }

Value2D operator+(const Value2D &a, const Value2D &b) { return Value2D(a._x + b._x, a._y + b._y); }

Value2D operator-(const Value2D &a, const Value2D &b) { return Value2D(a._x - b._x, a._y - b._y); }

Value2D operator-(const Value2D &a) { return Value2D(-a._x, -a._y); }
...
Value2D operator*(const Value2D &a, const double b) { return Value2D(a._x * b, a._y * b); }

Value2D operator*(const double a, const Value2D &b) { return b * a; }

double scalar_product(const Value2D &a, const Value2D &b) { return a._x * b._x + a._y * b._y; }
...
//
aContiBlock2D::aContiBlock2D() : isEvaluated(false) { Dprintf(("2Dblock-ctr")); }

aContiBlock2D::~aContiBlock2D() { Dprintf(("2Dblock-dtr")); }
...
double Integrator2D::special_input::Value() {  // interface class
    if (count == 0) a = in.Value();            // get vector input (x,y)
    switch (++count) {
        case 1:
            return a.x();  // first call (x)
        case 2:
            count = 0;  // second call (y)
            return a.y();
    }                         // switch
...
Integrator2D::Integrator2D(Input2D i) : _x(in), _y(in), in(i) {}
...
Integrator2D::Integrator2D() : _x(in), _y(in), in(Zero2D) {}
...
//
Value2D Integrator2D::Value() { return Value2D(_x.Value(), _y.Value()); }

////////////////////////////////////////////////////////////////////////////
//  aContiBlock2 --- base for 2 input blocks
//
aContiBlock2D2::aContiBlock2D2(Input2D i1, Input2D i2) : input1(i1), input2(i2) {
    if (input1 == this || input2 == this) SIMLIB_error(AlgLoopDetected);
    //  LoopCheck();
}

aContiBlock2D3::aContiBlock2D3(Input2D i1, Input2D i2, Input2D i3) : aContiBlock2D2(i1, i2), input3(i3) {
...
class _Add2D : public aContiBlock2D2 {
   ...
   public:
    _Add2D(Input2D a, Input2D b) : aContiBlock2D2(a, b) { Dprintf(("ctr: _Add2D[%p](in1,in2)", this)); }
...
class _Sub2D : public aContiBlock2D2 {
   ...
   public:
    _Sub2D(Input2D a, Input2D b) : aContiBlock2D2(a, b) { Dprintf(("ctr: _Sub2D[%p](in1,in2)", this)); }
    virtual Value2D Value() override {
        Value2D a = Input1Value();
        Value2D b = Input2Value();
        return a - b;
...
class _Mul2D1D : public aContiBlock2D1 {
   ...
   public:
    _Mul2D1D(Input2D a, Input b) : aContiBlock2D1(a), _b(b) { Dprintf(("ctr: _Mul2D1D[%p](in1,in2)", this)); }
    virtual Value2D Value() override {
...
class _Div2D : public aContiBlock2D1 {
   ...
   public:
    _Div2D(Input2D a, Input b) : aContiBlock2D1(a), _b(b) { Dprintf(("ctr: _Div2D[%p](in1_2D,in2)", this)); }
    virtual Value2D Value() override {
        Value2D a = InputValue();
        double b = _b.Value();
        return a / b;
...
class _UMinus2D : public aContiBlock2D1 {
   ...
   public:
    _UMinus2D(Input2D a) : aContiBlock2D1(a) { Dprintf(("ctr: _UMinus2D[%p](in)", this)); }
    virtual Value2D Value() override {
        Value2D a = InputValue();
        return -a;
    }
