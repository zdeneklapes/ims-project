...
//
ZDelayTimer::ZDelayTimer(double step, bool is_default) : dt(step), c(new ZDelayContainer) {  // constructor body
    if (is_default) {
...
ZDelayTimer::~ZDelayTimer() {
    if (ZDelay::default_clock == this) ZDelay::default_clock = 0;  // default timer deleted
    // clear all items
...
{                    // constructor body
    ...
    if (clock == 0) SIMLIB_internal_error();  // change ##############!!!!!
...
ZDelay::~ZDelay() {
    ...
    if (clock) clock->UnRegister(this);  // remove from delay list
}
