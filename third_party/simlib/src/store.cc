...
#define CHECKQUEUE(qptr)                        \
    do {                                        \
        if (!qptr) SIMLIB_error(QueueRefError); \
    } while (0)

////////////////////////////////////////////////////////////////////////////
//  constructors
//
Store::Store() : _Qflag(_OWNQ), capacity(1L), used(0L), Q(new Queue("Q")) { Dprintf(("Store::Store()")); }

Store::Store(unsigned long _capacity) : _Qflag(_OWNQ), capacity(_capacity), used(0L), Q(new Queue("Q")) {
    Dprintf(("Store::Store(%lu)", _capacity));
}

Store::Store(const char *name, unsigned long _capacity)
    : _Qflag(_OWNQ), capacity(_capacity), used(0L), Q(new Queue("Q")) {
    Dprintf(("Store::Store(\"%s\",%lu)", name, _capacity));
    ::SetName(this, name);
}

Store::Store(unsigned long _capacity, Queue *queue) : _Qflag(0), capacity(_capacity), used(0L), Q(queue) {
    CHECKQUEUE(queue);
    Dprintf(("Store::Store(%lu,%s)", _capacity, queue->Name().c_str()));
}

Store::Store(const char *name, unsigned long _capacity, Queue *queue)
    : _Qflag(0), capacity(_capacity), used(0L), Q(queue) {
    CHECKQUEUE(queue);
    Dprintf(("Store::Store(\"%s\",%lu, Queue\"%s\")", name, _capacity, queue->Name().c_str()));
    ::SetName(this, name);
}
...
void Store::SetCapacity(unsigned long newcapacity) {
    if (capacity < newcapacity || (QueueLen() == 0 && used <= newcapacity))
        capacity = newcapacity;
    else
        SIMLIB_error(SetCapacityError);
}
...
void Store::Enter(Entity *e, unsigned long rcap) {
    ...
    if (e != Current) SIMLIB_error(EntityRefError);  // current process only
...
void Store::Leave(unsigned long rcap) {
    ...
    while (pp != Q->end() && !Full()) {
        ...
        if (p->_RequiredCapacity > Free()) continue;  // skip if request can't be satisfied
        p->Out();                                     // remove from queue
        Dprintf(("%s.Enter(%s,%lu) from queue", Name().c_str(), p->Name().c_str(), p->_RequiredCapacity));
...
bool Store::OwnQueue() const { return (_Qflag & _OWNQ) != 0; }
