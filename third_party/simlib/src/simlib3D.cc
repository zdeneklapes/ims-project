...
double abs(const Value3D &a) { return sqrt(a._x * a._x + a._y * a._y + a._z * a._z); }

Value3D operator+(const Value3D &a, const Value3D &b) { return Value3D(a._x + b._x, a._y + b._y, a._z + b._z); }

Value3D operator-(const Value3D &a, const Value3D &b) { return Value3D(a._x - b._x, a._y - b._y, a._z - b._z); }

Value3D operator-(const Value3D &a) { return Value3D(-a._x, -a._y, -a._z); }

Value3D operator*(const Value3D &a, const Value3D &b) {
    return Value3D(a._y * b._z - a._z * b._y, a._z * b._x - a._x * b._z, a._x * b._y - a._y * b._x);
}

Value3D operator*(const Value3D &a, const double b) { return Value3D(a._x * b, a._y * b, a._z * b); }

Value3D operator*(const double a, const Value3D &b) { return b * a; }

double scalar_product(const Value3D &a, const Value3D &b) { return a._x * b._x + a._y * b._y + a._z * b._z; }
...
//
aContiBlock3D::aContiBlock3D() : isEvaluated(false) { Dprintf(("3Dblock-ctr")); }

aContiBlock3D::~aContiBlock3D() { Dprintf(("3Dblock-dtr")); }
...
Parameter3D &Parameter3D::operator=(const Value3D &x) {
    if (SIMLIB_Phase == SIMULATION) SIMLIB_error(ParameterChangeErr);
    value = x;
    return *this;
}
...
double Integrator3D::special_input::Value() {  // interface class
    if (count == 0) a = in.Value();            // get vector input
    switch (++count) {
        case 1:
            return a.x();  // first call (x)
        case 2:
            return a.y();  // second call (y)
        case 3:
            count = 0;
...
//
Integrator3D::Integrator3D(Input3D i, const Value3D &initial_value)
    : _x(in, initial_value.x()),  // linking of inputs to integrators
      _y(in, initial_value.y()),
      _z(in, initial_value.z()),
      in(i) {}

Integrator3D::Integrator3D(Input3D i) : _x(in), _y(in), _z(in), in(i) {}

// --- special copy constructor
Integrator3D::Integrator3D(Integrator3D &i, const Value3D &initial_value)
    : aContiBlock3D(),
      _x(in, initial_value.x()),  // linking of inputs to integrators
      _y(in, initial_value.y()),
      _z(in, initial_value.z()),
...
Integrator3D::Integrator3D() : _x(in), _y(in), _z(in), in(Zero3D) {}

////////////////////////////////////////////////////////////////////////////
// Integrator3D output method
//
Value3D Integrator3D::Value() { return Value3D(_x.Value(), _y.Value(), _z.Value()); }
...
//
aContiBlock3D2::aContiBlock3D2(Input3D i1, Input3D i2) : input1(i1), input2(i2) {
    if (input1 == this || input2 == this) SIMLIB_error(AlgLoopDetected);
    //  LoopCheck();
}

aContiBlock3D3::aContiBlock3D3(Input3D i1, Input3D i2, Input3D i3) : aContiBlock3D2(i1, i2), input3(i3) {
    if (input3 == this) SIMLIB_error(AlgLoopDetected);
}
...
class _Add3D : public aContiBlock3D2 {
   ...
   public:
    _Add3D(Input3D a, Input3D b) : aContiBlock3D2(a, b) { Dprintf(("ctr: _Add3D[%p](in1,in2)", this)); }
...
class _Sub3D : public aContiBlock3D2 {
   ...
   public:
    _Sub3D(Input3D a, Input3D b) : aContiBlock3D2(a, b) { Dprintf(("ctr: _Sub3D[%p](in1,in2)", this)); }
    virtual Value3D Value() override {
        Value3D a = Input1Value();
        Value3D b = Input2Value();
        return a - b;
...
class _Mul3D : public aContiBlock3D2 {
   ...
   public:
    _Mul3D(Input3D a, Input3D b) : aContiBlock3D2(a, b) { Dprintf(("ctr: _Mul3D[%p](in1,in2)", this)); }
    virtual Value3D Value() override {
        Value3D a = Input1Value();
        Value3D b = Input2Value();
        return a * b;
...
class _Mul3D1D : public aContiBlock3D1 {
   ...
   public:
    _Mul3D1D(Input3D a, Input b) : aContiBlock3D1(a), _b(b) { Dprintf(("ctr: _Mul3D1D[%p](in1,in2)", this)); }
    virtual Value3D Value() override {
...
class _Div3D : public aContiBlock3D1 {
   ...
   public:
    _Div3D(Input3D a, Input b) : aContiBlock3D1(a), _b(b) { Dprintf(("ctr: _Div3D[%p](in1_3D,in2)", this)); }
    virtual Value3D Value() override {
        Value3D a = InputValue();
        double b = _b.Value();
        return a / b;
...
class _UMinus3D : public aContiBlock3D1 {
   ...
   public:
    _UMinus3D(Input3D a) : aContiBlock3D1(a) { Dprintf(("ctr: _UMinus3D[%p](in)", this)); }
    virtual Value3D Value() override {
        Value3D a = InputValue();
        return -a;
    }
