...
void ContractStep(double time) {     // parameter is required end step time
    SIMLIB_ContractStepFlag = true;  // flag
    double newCS = time - SIMLIB_StepStartTime;
    if (newCS < SIMLIB_ContractStep) SIMLIB_ContractStep = newCS;      // can be only less
    if (newCS < SIMLIB_MinStep) SIMLIB_ContractStep = SIMLIB_MinStep;  // minimum
}
...
void SetAccuracy(double _abserr, double _relerr) {
    ...
    Dprintf(("SetAccuracy: maxerror = %g + %g * X ", SIMLIB_AbsoluteError, SIMLIB_RelativeError));
}

void SetAccuracy(double relerr) { SetAccuracy(0, relerr); }
...
void SIMLIB_ContinueInit() {
    ...
    if (IntegratorContainer::isAny() || StatusContainer::isAny() ||
        Condition::isAny()) {  // ------------------- initialize status --------------------
...
Integrator::Integrator() : input(SIMLIB_Integrator_0input) {
    Dprintf(("Integrator[%p]::Integrator()  #%d", this, IntegratorContainer::Size() + 1));
    CtrInit();
    initval = 0.0;
}
...
Integrator::Integrator(Input i, double initvalue) : input(i) {
    Dprintf(("Integrator[%p]::Integrator(Input,%g)  #%d", this, initvalue, IntegratorContainer::Size() + 1));
    CtrInit();
    initval = initvalue;
}
...
Integrator::Integrator(Integrator &i, double initvalue) : aContiBlock(), input(i) {
    Dprintf(
        ("Integrator[%p]::Integrator(Integrator[%p],%g) #%d", this, &i, initvalue, IntegratorContainer::Size() + 1));
    CtrInit();
    initval = initvalue;
}
...
Integrator::~Integrator() {
    Dprintf(("destructor: Integrator[%p]  #%d", this, IntegratorContainer::Size()));
    if (SIMLIB_DynamicFlag) {
...
double Status::Value() {
    if (!ValueOK) _Eval();  // if not, evaluate
