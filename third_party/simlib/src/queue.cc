...
//  constructors
//
Queue::Queue() { Dprintf(("Queue{%p}::Queue()", this)); }
...
//  destructor
//
Queue::~Queue() { Dprintf(("Queue{%p}::~Queue() // \"%s\" ", this, Name().c_str())); }
...
void Queue::PostIns(Entity *ent, iterator pos) {
    ...
    if (pos == end()) SIMLIB_internal_error();  // add error message
    PredIns(ent, ++pos);
}
...
Entity *Queue::Get(iterator pos) {
    ...
    StatN(size());
    StatN.n--;  // the number of samples correction
    return ent;
}
