...
begin_step:
    ...
    for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
        ...
        (*ip)->SetState((*ip)->GetOldState() + (106.0 * A1[i] - 408.0 * A3[i] + 352.0 * A4[i]) / 125.0);
    }
    ...
    for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
        ...
        (*ip)->SetState((*ip)->GetOldState() + 1.0 / 48.0 * A1[i] + 8.0 / 33.0 * A4[i] + 125.0 / 528.0 * A5[i]);
    }
    ...
    for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
        ...
        (*ip)->SetState((*ip)->GetOldState() - 1263.0 / 2401.0 * A1[i] + 39936.0 / 26411.0 * A4[i] -
                        64125.0 / 26411.0 * A5[i] + 5520.0 / 2401.0 * A6[i]);
    }
    ...
    for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
        ...
        (*ip)->SetState((*ip)->GetOldState() + 37.0 / 392.0 * A1[i] + 1625.0 / 9408.0 * A5[i] - 2.0 / 15.0 * A6[i] +
                        61.0 / 6720.0 * A7[i]);
    }
    ...
    for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
        ...
        (*ip)->SetState((*ip)->GetOldState() + 17176.0 / 25515.0 * A1[i] - 47104.0 / 25515.0 * A4[i] +
                        1325.0 / 504.0 * A5[i] - 41792.0 / 25515.0 * A6[i] + 20237.0 / 145800.0 * A7[i] +
                        4312.0 / 6075.0 * A8[i]);
    }
    ...
    for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
        ...
        (*ip)->SetState((*ip)->GetOldState() - 23834.0 / 180075.0 * A1[i] - 77824.0 / 1980825.0 * A4[i] -
                        636635.0 / 633864.0 * A5[i] + 254048.0 / 300125.0 * A6[i] - 183.0 / 7000.0 * A7[i] +
                        8.0 / 11.0 * A8[i] - 324.0 / 3773.0 * A9[i]);
    }
    ...
    for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
        ...
        (*ip)->SetState((*ip)->GetOldState() + 12733.0 / 7600.0 * A1[i] - 20032.0 / 5225.0 * A4[i] +
                        456485.0 / 80256.0 * A5[i] - 42599.0 / 7125.0 * A6[i] + 339227.0 / 912000.0 * A7[i] -
                        1029.0 / 4180.0 * A8[i] + 1701.0 / 1408.0 * A9[i] + 5145.0 / 2432.0 * A10[i]);
    }
    ...
    for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
        ...
        (*ip)->SetState((*ip)->GetOldState() - 27061.0 / 204120.0 * A1[i] + 40448.0 / 280665.0 * A4[i] -
                        1353775.0 / 1197504.0 * A5[i] + 17662.0 / 25515.0 * A6[i] - 71687.0 / 1166400.0 * A7[i] +
                        98.0 / 225.0 * A8[i] + 1.0 / 16.0 * A9[i] + 3773.0 / 11664.0 * A10[i]);
    }
    ...
    for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
        ...
        (*ip)->SetState((*ip)->GetOldState() + 11203.0 / 8680.0 * A1[i] - 38144.0 / 11935.0 * A4[i] +
                        2354425.0 / 458304.0 * A5[i] - 84046.0 / 16275.0 * A6[i] + 673309.0 / 1636800.0 * A7[i] +
                        4704.0 / 8525.0 * A8[i] + 9477.0 / 10912.0 * A9[i] - 1029.0 / 992.0 * A10[i] +
                        729.0 / 341.0 * A12[i]);
    }
    ...
    for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
        ...
        (*ip)->SetState((*ip)->GetOldState() + 31.0 / 720.0 * (A1[i] + A13[i]) + 16.0 / 75.0 * A6[i] +
                        16807.0 / 79200.0 * (A7[i] + A8[i]) + 243.0 / 1760.0 * (A9[i] + A12[i]));
    }
    ...
    for (ip = FirstIntegrator(), i = 0; ip != end_it; ip++, i++) {
        ...
        eerr = fabs(-1.0 / 480.0 * A1[i] - 16.0 / 375.0 * A6[i] - 2401.0 / 528000.0 * A7[i] +
                    2401.0 / 132000.0 * A8[i] + 243.0 / 14080.0 * A9[i] - 2401.0 / 19200.0 * A10[i] -
                    19.0 / 450.0 * A11[i] + 243.0 / 1760.0 * A12[i] + 31.0 / 720.0 * A13[i]);
        terr = fabs(SIMLIB_AbsoluteError) + fabs(SIMLIB_RelativeError * (*ip)->GetState());
