...
// constructor
//
Sampler::Sampler(void (*pf)(), double dt) : Next(0), function(pf), last(-1), step((dt > 0.0) ? dt : 0.0), on(true) {
...
// Output
//
void Sampler::Output() const { Print("Sampler%p, function=%p, last=%g, on=%i\n", this, function, last, on); }
...
//
void Sampler::Sample() {
    if (function) function();  // call global function
    last = Time;
}
