...
// AlgLoop  --  constructor
//
AlgLoop::AlgLoop(Input i, double eps, unsigned long max_it, double t_min, double t_max, double t0)
    : aContiBlock1(i),
      Eps(eps),
      MaxIt(max_it),
      TA(t_min),
...
//
void AlgLoop::Set(double eps, unsigned long max_it, double t_min, double t_max, double t0) {
    if (t_min >= t_max) {  // check boundary points
        SIMLIB_error(AL_BadBounds);
    }
...
//
void AlgLoop::Set(double eps, unsigned long max_it, double t_min, double t_max) {
    if (t_min >= t_max) {  // check boundary points
        SIMLIB_error(AL_BadBounds);
    }
    Eps = eps;
