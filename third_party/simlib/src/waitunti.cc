...
class WaitUntilList {
   ...
   public:
    ...
    static void create() {  // create single instance
        if (instance == 0)
            instance = new WaitUntilList;
        else
            SIMLIB_internal_error();  // called twice
...
void WU_print() {
    ...
    if (WaitUntilList::instance == 0) {
        _Print("none\n");
        return;
    }
    WaitUntilList::iterator i = WaitUntilList::begin();
    for (int n = 0; i != WaitUntilList::end(); ++i, ++n) _Print(" [%d] Process#%ld\n", n, (*i)->id());
}
#endif
...
void Process::_WaitUntilRemove() {
    if (_wait_until) WaitUntilList::Remove(this);
...
void WaitUntilList::InsertCurrent() {
    ...
    if (instance == 0) create();                           // create singleton instance
...
void WaitUntilList::GetCurrent() {
    ...
    if (empty()) INSTALL_HOOK(WUget_next, 0);  // uninstall hook if last item removed
...
void WaitUntilList::clear() {
    ...
    if (!instance->l.empty()) SIMLIB_internal_error();  // for sure
