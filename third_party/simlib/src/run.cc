...
SIMLIB_statistics_t::SIMLIB_statistics_t() { Init(); }
...
void SIMLIB_Init(double T0, double T1, unsigned version) {
    ...
    if (version != SIMLIB_version) {  // check versions
        Dprintf(("\n SIMLIB library version %x.%02x ", SIMLIB_version >> 8, SIMLIB_version & 0xFF));
...
void Run() {
    ...
    // first some checks
    if (SIMLIB_Phase != INITIALIZATION) SIMLIB_error(RunUseError);  // bad use of Run()
    if (NextTime < StartTime) SIMLIB_internal_error();              // never reached
    ...
    while (Time < EndTime && !StopFlag) {
        ...
        if (endFlag) _SetTime(NextTime, EndTime);  // limit NextTime to EndTime
    ...
    SQS::Clear();                          // terminate all scheduled events/processes
