...
//  constructors
//
List::List() : Link(this, this, this), n(0) { Dprintf(("List::List()")); }
...
void List::InsFirst(Link *ent) {
    if (ent->head != 0) SIMLIB_internal_error();
    ent->pred = this;
    ent->succ = Link::succ;
    Link::succ->pred = ent;
    Link::succ = ent;
...
void List::PostIns(Link *ent, iterator pos) {
    if (pos == end()) SIMLIB_error(ListActivityError);  /// change !!!!
    Link *act = *pos;
...
void List::PredIns(Link *ent, iterator pos) {
    if (ent->head != 0) SIMLIB_internal_error();
    Link *act = *pos;
    if (act->head != this) SIMLIB_error(ListActivityError);  /// change !!!!
    ent->pred = act->pred;
    ent->succ = act;
    act->pred->succ = ent;
    act->pred = ent;
...
//
Link *List::GetFirst() { return Get(begin()); }

////////////////////////////////////////////////////////////////////////////
// GetLast --- remove last item from list
//
Link *List::GetLast() { return Get(--end()); }

////////////////////////////////////////////////////////////////////////////
// Get --- remove selected item from list (virtual)
//
Link *List::Get(iterator pos) {
    if (empty()) SIMLIB_error(ListEmptyError);
    if (pos == end()) SIMLIB_error(ListActivityError);  // change !!
    Link *x = *pos;
    if (x->head != this) SIMLIB_error(LinkOutError);  // can't remove if not in list
