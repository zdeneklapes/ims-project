...
void SIMLIB_error(const char *filename, const int linenum) {
    _Print(_INT_ERR_TXT, (double)Time, _ErrMsg(InternalError), filename, linenum);
    _Print(_ABORT_TXT);
    SIMLIB_Phase = ERROREXIT;
...
void SIMLIB_warning(const enum _ErrEnum N) { _Print(_WARNING_TXT, (double)Time, _ErrMsg(N)); }
