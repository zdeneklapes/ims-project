...
void IntegrationMethod::Iterate(void) {
    ...
    while (1) {
        ...
        if (!SIMLIB_ContractStepFlag) break;
        if (SIMLIB_StepSize <= SIMLIB_MinStep) break;
...
bool IntegrationMethod::Prepare(void) {
    ...
    if (IsEndStepEvent) SIMLIB_StepSize = double(NextTime) - double(Time);
    ...
    if (SIMLIB_ResetStatus) {  // initialization of integration is requested
        ...
        if (SIMLIB_ConditionFlag) return false;  // condition has been changed => terminate step
    }

    if (SIMLIB_StepSize <= 0) SIMLIB_error(NI_IlStepSize);  // error of integration
...
void IntegrationMethod::SetOptStep(double opt_step) {  // set optimal step size
    SIMLIB_OptStep = opt_step;
}

void IntegrationMethod::SetStepSize(double step_size) {  // set step size
    SIMLIB_StepSize = step_size;
}

bool IntegrationMethod::IsConditionFlag(void) {  // any changes of condition vector?
    return SIMLIB_ConditionFlag;
}

int IntegrationMethod::GetErrNo(void) {  // return # of errors
    return SIMLIB_ERRNO;
}

void SetErrNo(int num) {  // set # of errors
    SIMLIB_ERRNO = num;
}
...
IntegrationMethod::Memory::Memory(std::list<Memory*>* PtrList) : arr(NULL), mem_size(0), ListPtr(PtrList) {
...
MultiStepMethod::MultiStepMethod(const char* name, const char* slave_name) : IntegrationMethod(name), Slave_Ptr(NULL) {
...
////////////////////////////////////////////////////////////////////////////
/// initialization
StatusMethod::StatusMethod(const char* name) : SingleStepMethod(name), PrevStatusNum(0) {
...
void StatusMethod::StatusResize(size_t size) {
    ...
    for (std::list<Memory*>::iterator it = StatusMList.begin(); it != StatusMList.end(); ++it) {
        (*it)->Resize(size);
    }
}  // StatusResize
...
void StatusMethod::StoreState(Memory& di, Memory& si, StatusMemory& xi) {
    ...
    for (ip = IntegratorContainer::Begin(), end_it = IntegratorContainer::End(), i = 0; ip != end_it; ++ip, i++) {
        di[i] = (*ip)->GetDiff();
        si[i] = (*ip)->GetState();
    }

    for (sp = StatusContainer::Begin(), status_end_it = StatusContainer::End(), i = 0; sp != status_end_it; ++sp, i++) {
        xi[i] = (*sp)->GetState();
    }
}  // StoreState
...
void StatusMethod::RestoreState(double dthlf, Memory& di, Memory& si, StatusMemory& xi) {
    size_t i;
    IntegratorContainer::iterator ip, end_it;
    StatusContainer::iterator sp, status_end_it;
