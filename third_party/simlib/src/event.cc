...
Event::Event(Priority_t p) : Entity(p) { Dprintf(("Event::Event(%u)", p)); }

////////////////////////////////////////////////////////////////////////////
///  destructor
Event::~Event() { Dprintf(("Event::~Event()")); }
...
void Event::Terminate() {
    ...
    if (isAllocated() && this != SIMLIB_Current) delete this;  // destroy entity (if not currently running Behavior)
}
...
std::string Event::Name() const {
    ...
    if (!name.empty())
        ...
        return SIMLIB_create_tmp_name("Event#%lu", _Ident);
}
#endif
