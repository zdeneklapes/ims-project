...
char tittle[] = "GENERR - error messages generator                     (c)PerPet 1991-1997 \n";
char help[] =
    "\n"
...
static char _Errors[] = {
    ...
    "Bad arguments\0"
    /* 2 */
    "File have not a header\0"
    /* 3 */
    "Error in head line\0"
    /* 4 */
    "Bad identifier\0"
    /* 5 */
...
FILE *OpenInputFile(char *name) {
    ...
    inf = fopen(name, "rt");
    if (inf == NULL) error(ErrCantOpenIn);
...
FILE *OpenOutputFile(char *name) {
    FILE *outf;
    outf = fopen(name, "wt");
    if (outf == NULL) error(ErrCantOpenOut);
    return outf;
}
...
void GenEnum(FILE *outf, TABLE *t) {
    int i;
    fprintf(outf, "enum %s {\n", enumname);
    for (i = 0; i < (*t).NumErr; i++) fprintf(outf, "/* %d */ %s,\n", i, (*t).Table[i].ErrId);
    fprintf(outf, "};\n\n");
}
