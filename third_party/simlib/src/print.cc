...
void SetOutput(const char *name) {
    if (OutFile != stdout) fclose(OutFile);  // close non-default
...
int Print(const double x) { return Print(" %g ", x); }

int Print(const double x, const double y) { return Print(" %g %g ", x, y); }

int Print(const double x, const double y, const double z) { return Print(" %g %g %g ", x, y, z); }
...
void Error(const char *fmt, ...) {
    ...
    fflush(OutFile);
    if (OutFile != stdout) vfprintf(stderr, fmt, argptr);
    va_end(argptr);
