...
inline void START_T() { T_MEASURE = rdtsc(); }
...
class Calendar {  // abstract base class
   ...
   public:
    ...
    static bool instance_exists() { return _instance != 0; }
...
struct EventNoticeLinkBase {    // base class with list pointers only
    ...
    void insert(EventNoticeLinkBase *p) {
        ...
        if (pred != this) remove();  // is in calendar, remove first
...
class CalendarList : public Calendar {
   ...
   public:
    ...
    /// dequeue
    virtual Entity *Get(Entity *p) override;  // remove process p from calendar
    /// dequeue first entity
    virtual Entity *GetFirst() override;
    /// remove all
    virtual void clear(bool destroy = false) override;  // remove/destroy all items
   ...
   public:
#ifndef NDEBUG
    virtual void debug_print() override;  // print of calendar contents - FOR DEBUGGING ONLY
#endif
};
...
inline EventNotice *EventNotice::Create(Entity *e, double t) {
    EventNotice *evn = e->GetEventNotice();
...
void CalendarList::ScheduleAt(Entity *e, double t) {
    ...
    if (t < Time) SIMLIB_error(SchedulingBeforeTime);
    l.insert(e, t);
    ++_size;
    // update mintime:
    if (t < MinTime()) SetMinTime(l.first_time());
}
...
Entity *CalendarList::Get(Entity *e) {
    ...
    if (Empty()) SIMLIB_error(EmptyCalendar);  // internal --> TODO:remove
    if (e->Idle()) SIMLIB_error(EntityIsNotScheduled);
...
//
// FIXME: TODO analyze possible problems with call in destructor
//
void CalendarList::clear(bool destroy) {
    Dprintf(("CalendarList::clear(destroy=%s)", destroy ? "true" : "false"));
    l.clear(destroy);
    _size = 0;
    SetMinTime(SIMLIB_MAXTIME);
}
...
class CalendarQueue : public Calendar {
   ...
   private:
    ...
    inline int time2bucket(double t) { return static_cast<int>(fmod(t / bucket_width, static_cast<double>(nbuckets))); }

    /// Compute bucket top limit
    // the *1.5 is good for bucket number >= 3
    // XXXX++++++--XXXX          X = time can be in bucket 1
    // 1111222233331111
    inline double time2bucket_top(double t) { return t + 1.5 * bucket_width; }
   ...
   public:
    ...
    virtual const char *Name() override { return "CalendarQueue"; }
...
void CalendarQueue::ScheduleAt(Entity *e, double t) {
    ...
    // if overgrown
    if (_size > LIST_MAX && list_impl()) switchtocq();
    ...
    if (list_impl()) {
        ...
        if (_size + 1 > hi_bucket_mark) Resize(+1);
...
Entity *CalendarQueue::GetFirst() {
    ...
    if (_size < LIST_MIN && !list_impl()) switchtolist();
    ...
    // update size
    --_size;
    if (_size < low_bucket_mark) Resize(-1);
    if (++numop > MAX_OP) Resize();
    // update mintime
...
Entity *CalendarQueue::Get(Entity *e) {
    ...
    if (Empty()) SIMLIB_error(EmptyCalendar);  // internal use only --> TODO:remove
    if (e->Idle()) SIMLIB_error(EntityIsNotScheduled);
...
void CalendarQueue::SearchMinTime(double starttime) {
    ...
    for (int n__ = nbuckets; n__ > 0; --n__) {  // n__ is not used
        ...
        ++nextbucket;
        if (nextbucket == nbuckets) nextbucket = 0;
...
double CalendarQueue::estimate_bucket_width() {
    ...
    for (int a__ = 0; a__ < 2; ++a__) {
        ...
        for (int n__ = nbuckets; n__ > 0; --n__) {  // n__ is not used
            ...
            // TODO: can be optimized if nbuckets is power of 2
            ++next_bucket;
            if (next_bucket == nbuckets) next_bucket = 0;
...
void CalendarQueue::Resize(int grow)  // TODO: is it better to use target size?
{
    ...
    // change nbuckets
    if (grow > 0) nbuckets = oldnbuckets * 2;
    if (grow < 0) nbuckets = oldnbuckets / 2;

    // check low limit
    if (nbuckets < MINBUCKETS) nbuckets = MINBUCKETS;  // minimal number of buckets is 4
...
void CalendarQueue::clear(bool destroy) {
    ...
    if (!Empty()) {
        if (list_impl())
            ...
            // empty all buckets
            for (unsigned i = 0; i < nbuckets; i++) buckets[i].clear(destroy);
        _size = 0;
    }
    // delete bucketarray
    delete[] buckets;
...
{
    Print("CalendarList:\n");
    if (CalendarList::instance_exists()) l.debug_print();
    Print("\n");
}
////////////////////////////////////////////////////////////////////////////
...
void SetCalendar(const char *name) {
    if (SIMLIB_Phase == INITIALIZATION || SIMLIB_Phase == SIMULATION)
        SIMLIB_error("SetCalendar() can't be used after Init()");
...
void SQS::ScheduleAt(Entity *e, double t) {  // used by scheduling operations
    if (!e->Idle()) SIMLIB_error("ScheduleAt call if already scheduled");
#ifdef MEASURE
    START_T();
#endif
