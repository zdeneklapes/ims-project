...
#define CHECKENTITY(fptr) \
    if (!fptr) SIMLIB_error(EntityRefError)
...
Facility::~Facility() {
    ...
    Clear();
    if (OwnQueue()) delete Q1;  // delete input queue
    delete Q2;
}
...
void Facility::Seize(Entity *e, ServicePriority_t sp) {
    ...
    CHECKENTITY(e);
    if (e != Current) SIMLIB_error(EntityRefError);
    e->_SPrio = sp;
    if (!Busy()) {
    ...
    if (sp > in->_SPrio) {  // special case: service interrupted
        ...
        Dprintf(("%s.Seize(%s,%u) from Q1", Name().c_str(), e->Name().c_str(), (unsigned)sp));
    }
}
...
void Facility::Release(Entity *e) {
    ...
    CHECKENTITY(e);
    if (!in) SIMLIB_error(ReleaseNotSeized);  // not seized
    if (e != in) SIMLIB_error(ReleaseError);  // seized by other entity
    in = NULL;                                // empty
    tstat(0);                                 // record
    tstat.n--;                                // correction !!
    ...
    if (!(Q1->empty() || Q2->empty())) {
        flag = (static_cast<Entity *>(Q1->front())->_SPrio > static_cast<Entity *>(Q2->front())->_SPrio);
    }
    ...
    {                           // seize from interrupt queue...
        ...
        Dprintf(("%s.Seize(%s,%u) from Q2", Name().c_str(), ent->Name().c_str(), (unsigned)ent->_SPrio));
        in = ent;  // seize again
        tstat(1);
        tstat.n--;                                  // correction !!!
...
void Facility::QueueIn2(Entity *e) {
    ...
    for (; p != Q2->end() && static_cast<Entity *>(*p)->_SPrio > ps;  // higher service priority first
         ++p) {                                                       /*empty*/
    }
    ServicePriority_t prio = e->Priority;
    for (; p != Q2->end() && static_cast<Entity *>(*p)->_SPrio == ps &&
           static_cast<Entity *>(*p)->Priority >= prio;  // higher priority first
         ++p) {                                          /*empty*/
    }
...
//  have own queue?
//
bool Facility::OwnQueue() const { return (_Qflag & _OWNQ1) != 0; }
